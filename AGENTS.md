...
- Upload only after explicit user intent. Dry-run must remain the default configuration.
...
A change is done when its public behavior is tested, failures are classified, secret handling is reviewed, docs match reality, dry-run still works, and no generated artifact or credential is committed.
